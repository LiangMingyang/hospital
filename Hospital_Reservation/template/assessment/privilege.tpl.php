 ...
 <div class="box102 p20" style="width: 100%">
	...
	<td>请选择数据库：
		...
		<?php foreach ($services as $serviceObj){
			if($targetServiceId==$serviceObj['ServiceId']){
				?>
				<option value="<?php echo $serviceObj['ServiceId']?>" selected="selected"><?php echo $serviceObj['ServerName'].":",$serviceObj['Protocol'].":".$serviceObj['ServiceName']?></option>
				<?php
			} else {
				?> 
				<option value="<?php echo $serviceObj['ServiceId']?>"><?php echo $serviceObj['ServerName'].":",$serviceObj['Protocol'].":".$serviceObj['ServiceName']?></option>
				<?php
			}
		}
			?>
