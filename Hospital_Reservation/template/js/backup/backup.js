...
function submitForm() {
	...
	if(selectValue==1) { //FTP服务器
		var bValid = true;
		var ipObj = $("#ftpIp");
		var socketObj = $("#ftpSocket");
		var pathObj = $("#ftpAllPath");
		var userNameObj = $("#ftpUsername");
		var passwordObj = $("#ftpPassword");
		...
		if(bValid) {
			...
			$.ajax({
				...
				data:{
					...
					ftpIp:ipObj.val(),
					ftpSocket:socketObj.val(),
					ftpPath:pathObj.val(),
					ftpUserName:userNameObj.val(),
					ftpPassword:passwordObj.val()
				},
...
//					window.location.href="backup.php";
		...
		var bValid = true;
		var localPathObj = $("#localDiskPath");
