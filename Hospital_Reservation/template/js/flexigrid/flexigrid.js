...
//							    cthch.addClass("noborder").click(function() {//add matychen 不要noborder，加入后ie8显示有问题。
			...
			populate: function () { //get latest data
				...
				//
//				if(p.newp == 1 && p.page!=1) p.newp=p.page;
...
//					   alert(JSON.stringify(data));
			...
			doSearch: function () {				
				//自定义查询
//				p.query=JSON.parse(p.getQuery());
				p.query = $.toJSON(p.getQuery());
//				p.newp = 1;
				p.newp=parseInt($('#currentPageHidden').val());
//				alert(p.newp);
				p.page = 1;
				this.populate();	
			},
			...
			ie9Fix: function() { //add by chenhuan@2013-05-16
