...
//					$('div:eq('+n+')',g.cDrag).css({'left':cdpos+'px'}).show();
			...
			showContent:function(i,data,tbody){
				var row = data.rows[i];
...
//				}
				...
				if(i>0){
					var idx = 0;
					var td = document.createElement('td');
					td.innerHTML = '<div style="text-align: left; width: 120px;">'+row.cell[idx]+'</div>';
					$(tr).append(td);
					td = null;
					
					idx = 1;
					td = document.createElement('td');
					td.innerHTML = '<div style="text-align: left; width: 300px;">'+row.cell[idx]+'</div>';
					$(tr).append(td);
					td = null;
				} else {
					var idx = 0;
...
//				    cthch.addClass("noborder").click(function() {//add matychen 不要noborder，加入后ie8显示有问题。
			    ...
			    if(i<data.rows.length) {
			    	setTimeout(function(){g.showContent(i,data,tbody)},1000); 
			    }
			},//---test end----
...
//					    });				
			...
			doSearch: function () {				
				//自定义查询
				//p.query=JSON.stringify(p.getQuery());
				p.query = $.toJSON(p.getQuery());
				p.newp = 1;
				this.populate();	
			},
		...
		$('#searchAllButton').click(function() {
			$("#searchAllHidden").val(1);
			g.doSearch();
		});
		$('#searchPreButton').click(function() {
			$("#searchAllHidden").val(0);
			g.doSearch();
		});
//		$('#searchButton2').click(function() {g.doSearch();});
		$('.search').click(function() {g.doSearch();});
		//add search button
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>搜索 : <input type='text' size='30' name='q' id='searchGridDate' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='显示全部数据' /></div>");
