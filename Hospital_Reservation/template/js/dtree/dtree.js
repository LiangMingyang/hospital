/* get relative path of the js */
var $jspath = "./";
var $jsname = "dtree.js";
var $cssname = "dtree.css";
var $jsnlen = $jsname.length;

var scripts = document.getElementsByTagName("script");
for(i = 0; i < scripts.length; i ++){
    if(scripts[i].src.substring(scripts[i].src.length - $jsnlen).toLowerCase() == $jsname.toLowerCase()){
        $jspath = scripts[i].src.substring(0,scripts[i].src.length - $jsnlen);
        break;
    }
}
...
createStyleSheet($jspath + $cssname);

// Node object
function Node(id, pid, name, canCheck, check, url, title, target, icon, iconOpen, open)
{
	this.id = id;
...
function dTree(objName)
{
	...
	this.icon = {root:$jspath+"img/base.gif", folder:$jspath+"img/folder.gif", folderOpen:$jspath+"img/folderopen.gif", node:$jspath+"img/page.gif", empty:$jspath+"img/empty.gif", line:$jspath+"img/line.gif", join:$jspath+"img/join.gif", joinBottom:$jspath+"img/joinbottom.gif", plus:$jspath+"img/plus.gif", plusBottom:$jspath+"img/plusbottom.gif", minus:$jspath+"img/minus.gif", minusBottom:$jspath+"img/minusbottom.gif", nlPlus:$jspath+"img/nolines_plus.gif", nlMinus:$jspath+"../images/tree/nolines_minus.gif"};
	this.obj = objName;
	this.aNodes = [];
	this.aIndent = [];
...
dTree.prototype.isChecked = function (nodeId)
{
    var node = this.getNode(nodeId);
    return node.check;
}

dTree.prototype.checkParentWhenInit = function () {
	for (var n = 0; n < this.aNodes.length; n++) {
		if(this.aNodes[n].check)
		{
			 this.checkParentNode(this.aNodes[n], true);
		}
	}
};

// Adds a new node to the node array
dTree.prototype.add = function (id, pid, name, canCheck, check, url, title, target, icon, iconOpen, open)
{
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, canCheck, check, url, title, target, icon, iconOpen, open);
};
...
dTree.prototype.checkNode = function (nodeId) {
	...
	else
	{
		...
		if(node.pNode)
			this.checkParentBySubNode(node.pNode);
	}
};
dTree.prototype.checkChildreNode = function (node, check) {
	for (var i = 0; i < node.cNode.length; i++)
	{
		...
		if (pCheck)
		{
			...
			this.getNode(node.cNode[i].id).check = check;
		}
...
dTree.prototype.checkParentBySubNode = function(node)
{
	if(node.id>0){
	var isChecked = false;
	for (var i = 0; i < node.cNode.length; i++)
	{
		...
		if (pCheck)
		{
			...
			{
				isChecked = true;
				document.getElementById(("c" + this.obj) + node.id).checked = true;
				break;
			}
		}
	}
	...
	if(!isChecked)
	{
		var check = document.getElementById(("c" + this.obj) + node.id);
		check.checked = false;
...
dTree.prototype.node = function (node, nodeId) {
	var str = "<div class=\"dTreeNode\" style='width:300px'>" + this.indent(node, nodeId);
	...
	if (this.config.useIcons) {
		...
		if (node.canCheck == 'true' || node.canCheck == true) {
			str += "<input id='c" + this.obj + node.id + "'onclick=\"" + this.obj + ".checkNode('" + node.id + "')\" type='checkbox' " + (node.check === "true" || node.check === true ? "checked" : "") + " />";
		} else {
	...
	if (node.url) {
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc)) {
			str += " href=\"javascript: " + this.obj + ".s(" + nodeId + ");policyVulShow('"+node.url+"');\"";
		}
		str += ">";
	} else {
