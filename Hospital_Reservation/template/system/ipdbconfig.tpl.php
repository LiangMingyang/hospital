	...
	if(checkHost()&&checkUserName()&&checkDBName()){
		$.ajax({
			...
			success:function(data){
				if(data=="OK") {
					artDialog({content:'修改成功！需要重新登录!', style:'succeed'}, function(){
						window.open('<?php echo WEB_ROOT.'system/login.php'?>', '_parent');
					});
				} else {
...
function isNull(str){
 ...
 <body>
 <h5 class="title102"><em>本地IP数据库配置</em></h5>
